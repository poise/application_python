...
action :before_compile do
  ...
  # the true command ensures the migration step passes eventhough the flask app may not require migration
  new_resource.migration_command "true" if !new_resource.migration_command
...
action :before_restart do
  ...
  ruby_block "remove_run_migration" do
    block do
...
def created_settings_file
  template "#{new_resource.path}/shared/#{new_resource.local_settings_base}" do
    source new_resource.settings_template || "flask_settings.py.erb"
    cookbook new_resource.settings_template ? new_resource.cookbook_name.to_s : "application_python"
    owner new_resource.owner
    group new_resource.group
    mode "644"
    variables :flask => new_resource, :debug => new_resource.debug, :settings => new_resource.settings
  end
end
